...
type storageUtil interface {
	...
	//创建索引，返回创建后的索引名字
	CreateIndex(index string)(string,error)
	//TODO 创建联合索引
	//删除索引
...
func  InitMgo(url string,database string,collection string) Mgo{
	...
	Collection := mgo.database.Collection(collection)
	mgo.collect = Collection
	return mgo
}
//插入一个文档
...
//创建单个唯一索引
func (mc *Mgo)CreateIndex(index string)(string,error){
	Doc:=bsonx.Doc{{index,bsonx.String("text")}}
...
//index名字
func (mc *Mgo) DropIndex(index string)error{
	_,err:=mc.collect.Indexes().DropOne(context.TODO(),index)
	if err != nil {
		log.Fatal(err)
	}
	return err
